...
This service also supports relaying Slack webhooks to n8n. When configuring your Slack app's webhooks, use the following URL format:

```
https://altiverr-webhook-relay.vercel.app/api/slack-webhook/<webhook-id>
```

Where `<webhook-id>` is the unique ID from your n8n webhook URL. For example, if your n8n webhook URL is:

```
http://localhost:5678/webhook/1910dd74-46e5-42f1-84b6-29be97f42d26/webhook
```

Your Slack webhook URL would be:

```
https://altiverr-webhook-relay.vercel.app/api/slack-webhook/1910dd74-46e5-42f1-84b6-29be97f42d26
```

### Slack URL Verification

The webhook relay automatically handles Slack's URL verification challenge by responding with the challenge parameter, allowing you to easily set up Event Subscriptions in your Slack app.

### Configuration

The Slack webhook relay requires the following environment variable:

- `N8N_WEBHOOK_URL` - The base URL for n8n webhooks (default: 'http://localhost:5678/webhook/')

When running n8n in Docker, you should set this environment variable to point to your Docker host's address and port for n8n. For example:

```
N8N_WEBHOOK_URL=http://localhost:5678/webhook/
```

Or if n8n is accessible on a different IP:

```
N8N_WEBHOOK_URL=http://192.168.1.100:5678/webhook/
```

### Testing the Slack Webhook

You can test the Slack webhook relay with the included test script:

```bash
# Test with regular event payload
./test-slack-webhook.sh [webhook-id]

# Test URL verification challenge
./test-slack-webhook.sh [webhook-id] challenge
```
...
curl -X POST \
  ...
  -d '{"event":"message_posted","text":"Hello from Slack!"}' \
